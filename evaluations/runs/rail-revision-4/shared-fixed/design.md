...
- 失敗：通信失敗/結果不明なら同じ操作を盲目的に再送せず、まずStock overviewとstock logを再読込して結果を確認する。Grocyの全操作idempotencyは保証しない。
...
合格：二人だけがloginでき、逐次操作に加え、同じ牛乳2を見たA `consume 1`・B `purchase 1`のほぼ同時確定後に最終2・対応transaction各1件となる。通信遮断で応答を見失った操作は、再送前にoverview/logから反映済み/未反映を一意に判定できる。さらにinventory訂正、50品目以下、backup/restoreが通る。停止：未認証閲覧、checkout>1,000円/月、期限入力を回避不能、競合後の算術/transaction不一致、結果不明を判定不能、restore不能。停止時は実データを増やさず自作再検討へ戻す。
