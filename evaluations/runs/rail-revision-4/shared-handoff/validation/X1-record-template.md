...
| 両requestのbarrier到着 / 解放時刻 | |
| A/B upstream開始・完了時刻 / 区間重複 | |
| A/B status・成功表示 / 追加mutation試行 | |
| 再取得count | |
...
password、email、URL全文、cookie、token、request body、実在庫名は記録しない。Network log/HAR/traceは保存しない。画面証拠は架空品目と結果だけへ安全化して私的保管し、この文書には証拠名とSHA-256だけを残す。
