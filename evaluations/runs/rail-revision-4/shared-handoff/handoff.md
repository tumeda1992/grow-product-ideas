...
6. **X1（実データ前の必須gate）**：[具体手順](validation/X1-procedure.md) の競合・応答消失を実行し、[記録型](validation/X1-record-template.md) から結果を保存する。不合格・確認不能なら実在庫を登録しない。通信を切っただけ、またはその瞬間の値が変わらないだけで成否を断定しない。
